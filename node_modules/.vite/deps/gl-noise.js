...
// node_modules/gl-noise/build/glNoise.m.node.js
...
var Worley = _Worley;
var BlendModes = _BlendModes;
var Common = _Common;
var GerstnerWave = _GerstnerWave;
var Curl = _Curl;
var _all = [Perlin, Simplex, Worley, BlendModes, GerstnerWave, Curl];
var All = _all;
var isNode = typeof process !== "undefined" && process.versions != null && process.versions.node != null;
async function nodeFetch(s) {
  const fs = await import("./fs-WXFRHYE3.js");
  const path = (await import("./path-FZY5TKI6.js")).default;
...
async function loadShadersRaw(shaders) {
  ...
  const output = await Promise.all(_shaders.map(async (s) => {
...
async function loadShaders(paths, chunks, headers) {
  ...
  if (!headers)
    headers = new Array(_paths.length).fill(Common);
...
function patchShaders(shader, chunks, headers) {
  ...
  if (!headers)
    headers = new Array(_shader.length).fill(Common);
  let output = _shader;
  if (chunks) {
...
//# sourceMappingURL=gl-noise.js.map
